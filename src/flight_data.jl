...
Operates on the NamedTuple returned by `load_flight_data`, which lives in
`V3KiteHDF5Ext` and needs `using HDF5`.
"""
...
Defined in `V3KiteHDF5Ext`; call `using HDF5` first to make it available.
"""
function load_flight_data(h5_path)
    error("load_flight_data needs HDF5 loaded: run `using HDF5` first.")
end
